...
public class PedestalRenderer implements BlockEntityRenderer<DMPedestalTile> {

	public PedestalRenderer(BlockEntityRendererProvider.Context context) {
	}

	@Override
	public void render(@Nonnull DMPedestalTile te, float partialTick, @Nonnull PoseStack matrix, @Nonnull MultiBufferSource renderer, int light, int overlayLight) {
		if (!te.isRemoved() && te.getLevel() != null) {
			if (Minecraft.getInstance().getEntityRenderDispatcher().shouldRenderHitBoxes()) {
				matrix.pushPose();
				BlockPos pos = te.getBlockPos();
				AABB aabb = te.getEffectBounds().move(-pos.getX(), -pos.getY(), -pos.getZ());
			...
			if (!stack.isEmpty()) {
				...
				Minecraft.getInstance().getItemRenderer().renderStatic(stack, TransformType.GROUND, light, overlayLight, matrix, renderer, (int) te.getBlockPos().asLong());
				matrix.popPose();
			}
		}
	}
