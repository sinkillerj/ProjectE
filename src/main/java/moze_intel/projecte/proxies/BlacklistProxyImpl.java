...
public class BlacklistProxyImpl implements IBlacklistProxy
{
    ...
    {
        Preconditions.checkState(Loader.instance().isInState(LoaderState.PREINITIALIZATION) || Loader.instance().isInState(LoaderState.INITIALIZATION), "Mod %s registering TimeWatch blacklist at incorrect time!", Loader.instance().activeModContainer().getModId());
