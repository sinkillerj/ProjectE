	...
	public CoordinateBox getBoxFromDirection(ForgeDirection direction, Coordinates coords, int range) {
		...
		if (direction.offsetX != 0) {
			if (direction.offsetX > 0)
				...
				return new CoordinateBox(coords.x, coords.y - 1, coords.z - 1, coords.x + range, coords.y + 1, coords.z + 1);
			...
			if (direction.offsetY > 0)
				...
				return new CoordinateBox(coords.x - 1, coords.y, coords.z - 1, coords.x + 1, coords.y + range, coords.z + 1);
		} else {
			if (direction.offsetZ > 0)
				return new CoordinateBox(coords.x - 1, coords.y - 1, coords.z - range, coords.x + 1, coords.y + 1, coords.z);
			else
				return new CoordinateBox(coords.x - 1, coords.y - 1, coords.z, coords.x + 1, coords.y + 1, coords.z + range);
		}
	}
