	...
	public CoordinateBox getBoxFromDirection(ForgeDirection direction, Coordinates coords) {
		...
		if (direction.offsetX != 0) {
			if (direction.offsetX > 0)
				...
				return new CoordinateBox(coords.x, coords.y - 1, coords.z - 1, coords.x + actualRange, coords.y + 1, coords.z + 1);
			...
			if (direction.offsetY > 0)
				...
				return new CoordinateBox(coords.x - 1, coords.y, coords.z - 1, coords.x + 1, coords.y + actualRange, coords.z + 1);
		} else {
			if (direction.offsetZ > 0)
				return new CoordinateBox(coords.x - 1, coords.y - 1, coords.z - actualRange, coords.x + 1, coords.y + 1, coords.z);
			else
				return new CoordinateBox(coords.x - 1, coords.y - 1, coords.z, coords.x + 1, coords.y + 1, coords.z + actualRange);
		}
	}
