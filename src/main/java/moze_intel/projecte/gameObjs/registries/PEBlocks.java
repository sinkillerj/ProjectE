...
public class PEBlocks {
	...
	public static final BlockRegistryObject<AlchemicalChest, BlockItem> ALCHEMICAL_CHEST = BLOCKS.register("alchemical_chest", () -> new AlchemicalChest(AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops().strength(10, 3_600_000)));
	...
	public static final BlockRegistryObject<Pedestal, BlockItem> DARK_MATTER_PEDESTAL = BLOCKS.register("dm_pedestal", () -> new Pedestal(AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops().strength(1, 3).lightLevel(state -> 12)), block -> new BlockItem(block, ItemDeferredRegister.getBaseProperties().fireResistant()));
	...
	public static final WallOrFloorBlockRegistryObject<InterdictionTorch, InterdictionTorchWall, WallOrFloorItem> INTERDICTION_TORCH = BLOCKS.registerWallOrFloorItem("interdiction_torch", InterdictionTorch::new, InterdictionTorchWall::new, AbstractBlock.Properties.of(Material.DECORATION).noCollission().strength(0).lightLevel(state -> 14).randomTicks());
	...
	public static final BlockRegistryObject<TransmutationStone, BlockItem> TRANSMUTATION_TABLE = BLOCKS.register("transmutation_table", () -> new TransmutationStone(AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops().strength(10, 30)));
	...
	private static BlockRegistryObject<Block, ItemFuelBlock> registerFuelBlock(String name, EnumFuelType fuelType) {
		return BLOCKS.registerDefaultProperties(name, () -> new Block(AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops()
	...
	private static BlockRegistryObject<Collector, CollectorItem> registerCollector(String name, EnumCollectorTier collectorTier, ToIntFunction<BlockState> lightLevel) {
		return BLOCKS.registerDefaultProperties(name, () -> new Collector(collectorTier, AbstractBlock.Properties.of(Material.GLASS).requiresCorrectToolForDrops()
				.harvestTool(ToolType.PICKAXE).strength(0.3F, 0.9F).lightLevel(lightLevel)), CollectorItem::new);
	}
	...
	private static <CONDENSER extends Condenser> BlockRegistryObject<CONDENSER, BlockItem> registerCondenser(String name,
		...
		return BLOCKS.register(name, () -> condenserFunction.apply(AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops()
	...
	private static BlockRegistryObject<Relay, RelayItem> registerRelay(String name, EnumRelayTier relayTier, ToIntFunction<BlockState> lightLevel) {
		return BLOCKS.registerDefaultProperties(name, () -> new Relay(relayTier, AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops()
	...
	private static BlockRegistryObject<ProjectETNT, BlockItem> registerExplosive(String name, TNTEntityCreator tntEntityCreator) {
		return BLOCKS.register(name, () -> new ProjectETNT(AbstractBlock.Properties.of(Material.EXPLOSIVE).strength(0), tntEntityCreator));
	}
	...
	private static BlockRegistryObject<MatterFurnace, BlockItem> registerFurnace(String name, EnumMatterType matterType, float hardness, float resistance) {
		return BLOCKS.register(name, () -> new MatterFurnace(AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops().strength(hardness, resistance)
	...
	private static BlockRegistryObject<MatterBlock, BlockItem> registerMatterBlock(String name, EnumMatterType matterType, float hardness, float resistance) {
		return BLOCKS.register(name, () -> new MatterBlock(AbstractBlock.Properties.of(Material.METAL).requiresCorrectToolForDrops().strength(hardness, resistance)
				.lightLevel(state -> 14), matterType), block -> new BlockItem(block, ItemDeferredRegister.getBaseProperties().fireResistant()));
	}
}