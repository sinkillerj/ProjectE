        ...
        if (this.isSmoking)
        {
            ...
            if (list.size() > 0)
            {
            	if (ent instanceof EntityPlayer)
            		worldObj.spawnEntityInWorld(new LootBall(worldObj, list, ent.posX, ent.posY, ent.posZ));
            	else worldObj.spawnEntityInWorld(new LootBall(worldObj, list, this.explosionX, this.explosionY, this.explosionZ));
            }
        }
    }
}
