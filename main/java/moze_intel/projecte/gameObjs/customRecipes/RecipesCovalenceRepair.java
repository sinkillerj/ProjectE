...
public class RecipesCovalenceRepair implements IRecipe
{
	...
	public boolean matches(InventoryCrafting inv, World world) 
	{
		...
		if (tool == null || !foundItem || dustCounter == 0) return false;
		if (dustCounter < 3) return false;
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (stack.getItemDamage() < dustDamage) return false;
		}
	...
	{
		if (stack.getHasSubtypes()) return false;
		if (stack.getMaxDamage() == 0 || stack.getItemDamage() == 0) return false;
		
		Item item = stack.getItem();
		if (item instanceof ItemShears || item instanceof ItemFlintAndSteel || item instanceof ItemFishingRod || item instanceof ItemBow)  return true;
	...
	private int getDustType(ItemStack stack)
	{
		...
		if (item instanceof ItemShears || item instanceof ItemFlintAndSteel) return 1;
		if (item instanceof ItemBow || item instanceof ItemFishingRod) return 0;
		...
		if (name.equals("WOOD") || name.toString().equals("STONE") || name.equals("CLOTH"))
			return 0;
